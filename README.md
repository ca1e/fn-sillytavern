...
- **桌面/手机图标经 fnOS 统一网关打开**（同源 + 飞牛登录态，手机 App 内嵌白屏问题已修复）
...
安装完成后，通过 fnOS 桌面的 **SillyTavern** 图标打开，走 **fnOS 统一网关**（`/app/fn-sillytavern`，与飞牛 Web 同源、复用飞牛登录态，桌面网页和手机 App 均可正常打开）。

同时保留局域网端口直连作为备用入口：`http://NAS地址:端口`（默认 `8000`，受白名单控制）。

> 说明：SillyTavern 前端使用大量绝对路径请求，无法直接挂在子路径下。本应用通过内置网关代理（`gateway-proxy.mjs`）注入前端适配脚本解决子路径适配，手机端请优先使用桌面图标入口；若手机浏览器直连 `http://NAS:8000` 白屏属于 webview 明文策略限制，改用图标入口即可。
...
| 桌面图标打开白屏/502 | 应用可能仍在启动，稍等刷新；若一直异常查看 `info.log` 中 `[gateway-proxy]` / `[main]` 日志 |
| 页面提示 403 Forbidden | 直连端口时你的 IP 不在白名单内，在应用配置页追加网段后保存（图标入口不受影响） |
| 改了端口打不开 | 配置向导保存后会自动重启；图标入口自动跟随，直连地址需手动更新 |

### 安全须知

- **桌面图标入口**走 fnOS 统一网关，需要飞牛登录态，安全性最佳
- **端口直连入口**由 SillyTavern 白名单控制（`config.yaml` 的 `whitelist` 字段，支持 CIDR），请勿将端口暴露到公网；如需远程使用，建议通过 VPN / Tailscale 等组网方式，并在白名单中放行对应网段
